...
#endif
#if defined(CONFIG_SPLASHIMAGE)
	"splashimage="	MK_STR(CONFIG_SPLASHIMAGE)	"\0"  
#endif
...
uchar env_get_char_memory (int index)
{
    if (gd->env_valid)
    {
       // printf("env_get_char_memory gd->env_valid \n");
		return ( *((uchar *)(gd->env_addr + index)) );

    } else
    {
        //printf("env_get_char_memory  default_environment \n");
