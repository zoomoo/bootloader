...
/*---------------------------------------------------------------------*/
#if	defined(UBOOT_LCD_BACKLIGHT)
void lcd_backlight_ctrl (uint bk_status)
{
	uint reg_f_cfg=readl(REG_GPFCON) & 0xEFFFFFFF | (1<<28);
        writel(reg_f_cfg,REG_GPFCON);
        uint reg_f_on=readl(REG_GPFDAT) & 0xFFFFBFFF | (1<<14);
        uint reg_f_off=readl(REG_GPFDAT) & 0xFFFFBFFF;
	uint reg_f_sta=bk_status?reg_f_on:reg_f_off;
	writel(reg_f_sta,REG_GPFDAT);
}
#endif
/*----------------------------------------------------------------------*/
...
int drv_lcd_init (void)
{
	...
	if(((s=getenv("bootargs"))!=NULL)&&((size=(strstr(s,"lcdsize=")))!=NULL))
	{
		if(strncmp(size,"lcdsize=35",10)==0)
		{	
			panel_info.vl_col=320;
			panel_info.vl_row=240;
			lcd_size_in_env="35";
		}
		else if(strncmp(size,"lcdsize=43",10)==0)
		{		
			panel_info.vl_col=480;
			panel_info.vl_row=272;
			lcd_size_in_env="43";
		}
		else if(strncmp(size,"lcdsize=50",10)==0)
		{		
			panel_info.vl_col=800;
			panel_info.vl_row=480;
			lcd_size_in_env="50";
		}
		else if(strncmp(size,"lcdsize=56",10)==0)
		{		
			panel_info.vl_col=640;
			panel_info.vl_row=480;
			lcd_size_in_env="56";
		}
		else if(strncmp(size,"lcdsize=70",10)==0)
		{		
			panel_info.vl_col=800;
			panel_info.vl_row=480;
			lcd_size_in_env="70";
		}
		else if(strncmp(size,"lcdsize=80",10)==0)
		{		
			panel_info.vl_col=800;
			panel_info.vl_row=600;
			lcd_size_in_env="80";
		}
		else if(strncmp(size,"lcdsize=VGA800",10)==0)
		{		
			panel_info.vl_col=800;
			panel_info.vl_row=600;
			lcd_size_in_env="VGA800";
		}
		else
		{		
			panel_info.vl_col=480;
...
#if	(defined(UBOOT_LCD_BACKLIGHT)&&defined(CONFIG_SPLASH_SCREEN))||(defined(UBOOT_LCD_BACKLIGHT)&&defined(CONFIG_LCD_OUTPUT))
...
static int lcd_init (void *lcdbase)
{
	...
	if(lcd_size_in_env!=NULL)
	{
		if(strncmp(lcd_size_in_env,"35",2)==0)
		{		
			lcd_ctrl_init_35 (lcdbase);
		}
		else if(strncmp(lcd_size_in_env,"43",2)==0)
		{		
			lcd_ctrl_init (lcdbase);
		}
		else if(strncmp(lcd_size_in_env,"50",2)==0)
		{		
			lcd_ctrl_init_50 (lcdbase);
		}
		else if(strncmp(lcd_size_in_env,"56",2)==0)
		{		
			lcd_ctrl_init_56 (lcdbase);
		}
		else if(strncmp(lcd_size_in_env,"70",2)==0)
		{		
			lcd_ctrl_init_70 (lcdbase);
		}
		else if(strncmp(lcd_size_in_env,"80",2)==0)
		{		
			lcd_ctrl_init_8_10 (lcdbase);
		}
		else if(strncmp(lcd_size_in_env,"VGA800",2)==0)
		{		
			lcd_ctrl_init_vga800 (lcdbase);
		}
		else
		{
			lcd_ctrl_init (lcdbase);
...
int lcd_display_bitmap(ulong bmp_image, int x, int y)
{
	...
	ushort *cmap;
#endif
	bmp_color_table_entry_t cmap_t[280];
	ushort i, j;
	//uchar *fb;
	bmp_color_table_entry_t *fb;
...
//# error "Don't know location of color map"
	...
	fb   = (bmp_color_table_entry_t *) (lcd_base +
		(y + height - 1) * lcd_line_length + x);
	for (i = 0; i < height; ++i) {
		WATCHDOG_RESET();
		for (j = 0; j < width ; j++)
...
static void *lcd_logo (void)
{
	...
	if (do_splash && (s = getenv("splashimage")) != NULL) {
		...
		do_splash = 0;
		sprintf(cmd, "nand read.i 0x50008000 %x %x", addr,CONFIG_SPLASH_SCREEN_SIZE);
		ExecuteCmd(cmd);
		addr=0x50008000;
