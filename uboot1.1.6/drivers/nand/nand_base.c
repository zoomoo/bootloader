                           ...
                           const uint8_t *buf, int page, int cached, int raw)
{
    ...
    //    printf("s3c_nand_write_page_8bit\n");
    ...
    } else
    {
        if (unlikely(raw))
        {
            ...
            // printf("chip->ecc.write_page_raw(mtd, chip, buf \n");
        }
        else
        {
            chip->ecc.write_page(mtd, chip, buf);
            // printf("chip->ecc.write_page(mtd, chip, buf)\n");
        }
                    ...
                    int allowbbt)
{
    ...
    // printf("nand_erase: start = 0x%08x, len = %i\n",(unsigned int)instr->addr, (unsigned int)instr->len);
    // printf("chip->phys_erase_shift=%d\n",chip->phys_erase_shift);
    // printf("1<<chip->phys_erase_shift=%d \n",1<<chip->phys_erase_shift);
    ...
    while (len) {
        ...
        if (status & NAND_STATUS_FAIL) {
            ...
            //    printf("nand_erase: Failed erase, page 0x%08x\n", page);
...
erase_exit:
    ...
    for (chipnr = 0; chipnr < chip->numchips; chipnr++) {
        ...
        printf("nand_erase_nand: nand_update_bbt (%d:0x%0x 0x%0x)\n", chipnr, rewrite_bbt[chipnr],
               chip->bbt_td->pages[chipnr]);
...
//    printf("maf_id=%x,dev_id=%x\n",*maf_id,dev_id);
    ...
    int cellinfo = chip->read_byte(mtd);	/* 3rd byte */
    int importantInfo = chip->read_byte(mtd);		/* 4th byte */

    //int childType=importantInfo & 0x03; //Page size
    int childType=cellinfo;
    ...
    if (!type->pagesize)
    {
        ...
        chip->cellinfo = cellinfo;// chip->read_byte(mtd);
        /* The 4th id byte is the important one */
        extid = importantInfo;//chip->read_byte(mtd);
        /* Calc pagesize */
...
#ifdef FORLINX_DEBUG
        printf("Calc pagesize,Calc blocksize,Calc erasesize.............\n");
    ...
    } else
    {
         ...
         */
        if(dev_id == 0xd5 && childType==0x94)//K9GAG08U0D
        {
            mtd->oobsize = 128;

        }else if(dev_id == 0xd5 && childType==0x14)//K9GAG08U0M
        {
            mtd->oobsize = 128;
        }
        else if(dev_id == 0xd5 && childType==0x84) //K9GAG08U0E
        {
            type->pagesize=8192;
            type->chipsize=2048;
            type->erasesize=1024*1024;

            mtd->oobsize = 232;

        }else if(dev_id == 0xd7)//K9LBG08UOD
        {
            mtd->oobsize = 128;

        }else if(*maf_id==0x2c && dev_id==0x48)//MT29F16G08ABACAWP
        {
            mtd->oobsize = 128;
        }
        else if(*maf_id==0x2c && dev_id==0x38)//MT29F8G08ABABAWP
        {
            mtd->oobsize = 128;
        }
	else if(*maf_id==0xec && dev_id==0xd3)//K9K8G08U0E
        {
            mtd->oobsize = 64 ;
        }
	else
...
#ifdef FORLINX_DEBUG
        printf("No  No Calc pagesize, blocksize, erasesize,  use ids table .............\n");
#endif
    }
